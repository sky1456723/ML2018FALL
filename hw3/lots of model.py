...
for i in range(5):
    ...
    model.add(keras.layers.Dense(units=300, kernel_initializer = keras.initializers.he_normal()) )
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.Activation('relu'))
    model.add(keras.layers.Dropout(0.3))
    model.add(keras.layers.Dense(units=300, kernel_initializer = keras.initializers.he_normal()) )
    ...
    history = model.fit_generator(augmentData.flow(training, training_label, batch_size = 100, seed = 0),
                       ...
                       epochs = 300)#, callbacks = [early])

    model.save("./model/lots_of_cnn_"+str(i)+".h5")
    
    loss_h = np.array(history.history['loss'])
    np.savetxt("./log/training_loss_"+str(i)+".txt",loss_h)
    acc_h = np.array(history.history['acc'])
    np.savetxt("./log/training_acc_"+str(i)+".txt",acc_h)
    val_acc_h = np.array(history.history['val_acc'])
    np.savetxt("./log/val_acc_"+str(i)+".txt",val_acc_h)
    val_loss_h = np.array(history.history['val_loss'])
    np.savetxt("./log/val_loss_"+str(i)+".txt",val_loss_h)
